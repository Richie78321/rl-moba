...
#1 is healer, 2 is tank, 3 is damage
...
async def run(env: DerkSession, app: DerkAppInstance):
    ...
    for iteration in range(ITERATIONS):
        ...
        new_configs = [{"slots": [random.choice(arm_weapons[i%3]), random.choice(misc_weapons[i%3]), random.choice(tail_weapons[i%3])]} for i in range(3 * n_arenas)]
...
async def main():
    random_start = [{"slots": [random.choice(arm_weapons[i%3]), random.choice(misc_weapons[i%3]), random.choice(tail_weapons[i%3])]} for i in range(3 * n_arenas)]
    app = DerkAppInstance()

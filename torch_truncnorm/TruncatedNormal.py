...
class TruncatedStandardNormal(Distribution):
    ...
    def __init__(self, device, a, b, eps=1e-8, validate_args=None):
        self.device = device
        ...
        self._dtype_min_gt_0 = torch.tensor(torch.finfo(self.a.dtype).eps, dtype=self.a.dtype).to(self.device)
        self._dtype_max_lt_1 = torch.tensor(1 - torch.finfo(self.a.dtype).eps, dtype=self.a.dtype).to(self.device)
    ...
    def rsample(self, sample_shape=torch.Size()):
        ...
        p = torch.empty(shape).to(self.device).uniform_(self._dtype_min_gt_0, self._dtype_max_lt_1)
        return self.icdf(p)
...
class TruncatedNormal(TruncatedStandardNormal):
    ...
    def __init__(self, device, loc, scale, a, b, eps=1e-8, validate_args=None):
        ...
        super(TruncatedNormal, self).__init__(device, a_standard, b_standard, eps=eps, validate_args=validate_args)
